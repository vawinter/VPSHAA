...
table4 <- data.frame("Covariates" = c("Elevation", "Roughness", "Aspect (Easting)", "Aspect (Northing)",
                                      ...
                                      "Tree cover"),
                     # Full mod
                     "R²" = c("57", "43", "12", "33", "21", "50", "23"),
                                         # Seas mod
                     "R²" = c("02", "20", "02", "20", "01", "12", "06"),
                                          # Null mod
                     "R²" = c("0", "0", "0", "0", "0", "0", "0"),
                                          check.names = FALSE) %>% 
  knitr::kable(booktabs = T,
 ...
 # add_header_above(c(" ", "marginal" = 1, "conditional" = 3), align = "r") %>% 
  add_header_above(c(" ", "Full model" = 1, "Season Model" = 1, "Null Model" = 1)) %>% 
  kable_styling(latex_options = c("hold_position", "repeat_header", "striped"), full_width = FALSE)
...
table6 <- data.frame("Season" = c("Winter", "Winter", "Spring", "Spring", "Spring", 
                     ...
                     "Rank value" = c("0.08", "0.15", "0.15", "0.33", "0.32", "0.22", "0.24", "0.04", "0.64", "0.30",
                                      "0.39", "-0.18", "0.27", "0.32"),
                    # combined
                     "Rank value" = c("0.47", "0.27", "0.83*", "-0.62", "\textbf{0.85*}", "0.93*",  "0.19", "-0.58", "\textbf{0.89*}",
                                          "\textbf{0.77*}", "\textbf{0.92*}", "0.37", 
                                      "\textbf{0.92*}", "\textbf{0.78*}"),
                         check.names = FALSE) %>% 
  knitr::kable(booktabs = T,
               escape = F,
               ...
               * denotes values with greater than 0.5 positive values in combined-order rank",
               format = "latex",
               align = c("lrccccccccc")) %>% 
 # kable_styling(font_size = 30, html_font = "Arial Nova") %>% 
