...
table4 <- data.frame("Covariates" = c("Elevation", "Roughness", "Aspect (Easting)", "Aspect (Northing)",
                                      ...
                                      "Tree cover"),
                     # Full mod
                     "Full" = c("57", "43", "12", "33", "21", "50", "23"),
                    # "R² (conditional)" = c("99", "99", "99", "99", "99", "99", "99"),
                     # Seas mod
                     "Season-only" = c("02", "20", "02", "20", "01", "12", "06"),
                     #"R² (conditional)" = c("99", "99", "99", "99", "99", "99", "99"),
                     # Null mod
                     "Null" = c("0", "0", "0", "0", "0", "0", "0"),
                     #"R² (conditional)" = c("99", "99", "99", "99", "99", "99", "99"),
                     check.names = FALSE) %>% 
  knitr::kable(booktabs = T,
               escape = F,
               caption = "Goodness of fit: R 
               percentage values for each Full, Null, and Season-only covariate model output.",
               format = "latex",
               align = c("lrccccccccc")) %>% 
  #column_spec(2,bold=T,latex_column_spec = ">{\\\\color{black}}c") %>% 
 # collapse_rows(columns = 2, latex_hline = "major",valign = "middle") %>%
...
table5 <- data.frame("Selection Coefficients" = 
                       c("Elevation", "Roughness","Aspect (Easting)", "Aspect (Northing)",
                                         "Herbaceous cover", "Shrub cover", "Tree cover"),
                     # Full
                     "Full" = c("0.088", "0.255", "-0.036", "0.336","0.040", "0.156","0.223"),
                     # Season
                     "Season" = c("-0.103", "0.003","0.043", "0.360", "-0.045", "0.353","0.0293"),
                     # Null
                     "Null" = c("8.20E-17", "-7.46E-17","-3.89E-16", "3.90E-16", "-1.44E-16", "4.14E-16","-2.12E-16"),
                     # Full and season
                     "(Full vs Season)" = c("0.095", "0.126", "-0.012", "-0.022", "0.043", "-0.087", "-0.035"),
                     "(Full vs Null)" = c("0.044", "0.127", "-0.017", "0.158", "0.020", "0.078", "0.112"),
                     check.names = FALSE) %>% 
  knitr::kable(booktabs = T,
               escape = F,
               # caption = "Out of sample validation: Pearson's weighted correlation estimations
...
table6 <- data.frame("Season" = c("Winter", "Winter", "Spring", "Spring", "Spring", 
                                  ...
                                  "Fall",   "Fall",   "Fall",   "Fall"),
                     # status
                     "Status" = c("Ranging", "Ranging", "Resident", "Resident", "Mover",    
