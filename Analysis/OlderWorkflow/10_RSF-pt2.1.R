...
#                                                                   "202111_model_data.rds",
                                                                    "20231130_covaraite_avail.rds",
                                                                 "20221011_3rd-order_RSF-prep.rds",
                                                                 "20221018_3rd-order_RSF-prep.rds",
                                                                 "20230812_3rd-order_RSF-prep.rds",
                                                                 "20230812_measn-sd-preRSF.csv")]

# merge into one file
# File lengths are different. Loading them in by season to investigate why
all_sum <- do.call(rbind, lapply(files[c(3,7,11,15)], readRDS)) 
all_win <- do.call(rbind, lapply(files[c(1,5,9,13)], readRDS)) 
all_spr <- do.call(rbind, lapply(files[c(2,6,10,14)], readRDS)) 
all_fall <- do.call(rbind, lapply(files[c(4,8,12,16)], readRDS)) 
...
# change to data frame, not xyt object 
class(dat)
dat <- as.data.frame(dat, row.names = T)
...
# Save output ----
saveRDS(dat, "Data/Processed/RSF_data/20231130_covaraite_avail.rds")
#saveRDS(dat, "Data/Processed/RSF_data/20230812_3rd-order_RSF-prep.rds")
...
write.csv(mod_fin, "Data/Processed/RSF_data/20230812_measn-sd-preRSF.csv", row.names = F)
